...
int EMSCRIPTEN_KEEPALIVE GetWavLength() {
    int bufferLength = GetBufferLength();
    return bufferLength + 44;
}

char* EMSCRIPTEN_KEEPALIVE Speak(char *input) {
    //char* argv = "hi im sam";
    //InitArgs(3, &argv);

    strcat_s((char*)input, 256, "[");
    if (!TextToPhonemes(input)) return NULL;
	SetInput(input);
    ...
    int wavLength = GetWavLength();
	char *wavBuffer = malloc(wavLength); 
	...
	FILE *file;
	file = fmemopen(wavBuffer, wavLength, "wb");
    ...
    int dLength = bufferLength/50;
	fwrite(&dLength, 4, 1, file);
